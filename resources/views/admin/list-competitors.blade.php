       ...
       @include('layouts.partials.menu')
        ...
        <div class="page-header">
            <div class="page-leftheader">
                <h4 class="page-title">Competitors</h4>
                <ol class="breadcrumb pl-0">
                    <li class="breadcrumb-item"><a href="#">Admin</a></li>
                    <li class="breadcrumb-item"><a href="#">Competitors</a></li>
                </ol>
            </div>
            <div class="page-rightheader">
                @permission('add-competitor')
                <button type="button" class="btn btn-sm btn-primary mr-1"><i class="fe fe-plus mr-2"></i><a
                        href="{{ route('admin.competitors') }}" class="text-white">Add
                        Competitor</a></button>
                        @endpermission
                    ...
                    <table id="example" class="table table-striped table-bordered" style="width:auto;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $row->name }}</td>
                                <td class="text-right">
                                    @permission('eidt-competitor')
                                    <a title="Edit" href="edit-competitors/{{$row->id}}" class="btn btn-sm btn-success">
                                        <i class="fa fa-pencil"></i>
                                    </a>
                                    @endpermission
                                    @permission('delete-competitor')
                                    <a title="Delete" href="javascript:void(0)"  onclick="delete_competitors('{{$row->id}}')" type="button"
...
function delete_competitors(id) {
    swal({
        ...
        if (willDelete) {
            $.ajax({
                type: "POST",
                url: '{{route("admin.delete-competitors")}}',
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
                },
                success: function(data) {
                    if (data == 1) {
                        window.location = 'list-competitors';
                    }
                },
