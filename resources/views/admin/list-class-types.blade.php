    ...
    <div class="container">
        ...
        <div class="page-header">
            ...
            <div class="page-rightheader">
                @permission('add-classtypes')
                <button type="button" class="btn btn-sm btn-primary mr-1"><i class="fe fe-plus mr-2"></i><a href="{{ route('admin.class-types') }}"
                        ...
                        @foreach($data as $row)
                            <tr role="row" class="even">
                                ...
                                <td class="text-right">
                                    ...
                                    @permission('eidt-classtypes')
                                    <a  href="edit-class-types/{{$row->id}}" class="btn btn-sm btn-success">
                                        <i class="fa fa-pencil"></i>
                                        Edit
                                    </a>
                                    @endpermission
                                    @permission('delete-classtypes')
                                    <a href="javascript:void(0)" onclick="delete_category('{{$row->id}}')"  class="btn btn-sm btn-danger">
...
function delete_category(id) {
    swal({
        ...
        if (willDelete) {
            $.ajax({
                type: "POST",
                url: '{{route("admin.delete-class-types")}}',
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
                },
                success: function(data) {
                    if (data == 1) {
                        window.location = 'list-class-types';
                    }
                },
