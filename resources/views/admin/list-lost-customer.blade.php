    ...
    <div class="container">
        ...
        <div class="page-header">
            ...
            <div class="page-rightheader">
                @permission('add-lost-customer-reason')
                <button type="button" class="btn btn-sm btn-primary mr-1"><i class="fe fe-plus mr-2"></i><a href="{{ route('admin.lostcustomer') }}"
                        ...
                        @foreach($data as $row)
                            <tr>
                                ...
                                <td class="text-right">
                                    ...
                                    @permission('eidt-lost-customer-reason')
                                    <a href="edit-lostcustomer/{{$row->id}}" class="btn btn-sm btn-success">
                                        <i class="fa fa-pencil"></i>
                                        Edit
                                    </a>
                                    @endpermission
                                    @permission('delete-lost-customer-reason')
                                    <a href="javascript:void(0)" onclick="delete_lost('{{$row->id}}')" type="button" class="btn btn-sm btn-danger">
...
function delete_lost(id) {
    swal({
        ...
        if (willDelete) {
            $.ajax({
                type: "POST",
                url: '{{route("admin.delete-lostcustomer")}}',
                data: {
                    id: id,
                    _token: '{{csrf_token()}}'
                },
                success: function(data) {
                    if (data == 1) {
                        window.location = 'list-lostcustomer';
                    }
                },
