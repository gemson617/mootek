                ...
                <div class="col-sm-12">
                    <div class="card border-left-primary shadow h-100 ">
                        <div class="card-header">
                            <h3 class="card-title">GPS Platform</h3>
                        </div>
                        ...
                        <div class="card-body">
                            ...
                            <form id="form" method="post" action="{{route('status.storeStatus')}}">
                                ...
                                <div class="row">
                                    <input type="hidden" name="table" value="GPS" />
                                    <div class="col-md-4">
                                        <label for="">GPS Platform<span class="error">*</span></label>
                                        <div class="form-group">
                                            <input type="Text" id='gps_platform_name' class="form-control txt-num @error('gps_platform_name') is-invalid @enderror" name="gps_platform_name" placeholder="GPS Platform Name" />
                                            @error('gps_platform_name')
                       ...
                       <?php if(isset($status)){ 
                        ...
                        <tr>
                            ...
                            <td><a href="javascript:void(0)" data-id="<?= $val->id ?>" class="btn btn-primary edit_form "><i class="fa fa-edit"></i></a>
                                ...
                                <button value='{{$val->id}}' data="{{$val->status}}" data-name="GPS" class='btn btn-info change'>status</button>
                            </td>
                        </tr>
                        <?php $i++; }  } ?>
                    </tbody>
...
<script>
        ...
        rules: {
            gps_platform_name: 'required',
            status: 'required',
        },
        messages: {
            gps_platform_name: 'This GPS Platform Name is Required',
            ...
            $.ajax({
                ...
                success: function(data) {   
                    if(data != 1){
                        ...
                        console.log(row);                    
                        $("#gps_platform_name").val(row.gps_platform_name);
                        $("#status").val(row.status).select2();     
                        $("#id").val(row.id);           
                        $(".card-title").html('Edit GPS Platform').css('color','blue');                  
                        $("#payment_mode").focus();
                        scrollToTop();
                    }               
                },
