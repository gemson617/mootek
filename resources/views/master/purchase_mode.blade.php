                ...
                <div class="col-sm-12">
                    <div class="card border-left-primary shadow h-100 ">
                        <div class="card-header">
                            <h3 class="card-title">Purchase Mode</h3>
                        </div>
                        ...
                        <div class="card-body">
                            ...
                            <form id="form" method="post" action="{{route('status.storeStatus')}}">
                                ...
                                <div class="row">
                                    <input type="hidden" name="table" value="purchase_mode" />
                                    <div class="col-md-4">
                                        <label for="">Purchase Mode<span class="error">*</span></label>
                                        <div class="form-group">
                                            <input type="Text" placeholder="Purchase Mode" id='purchase_mode' class="form-control txt-num @error('purchase_mode') is-invalid @enderror" name="purchase_mode" />
                                            @error('purchase_mode')
                       ...
                       <?php if(isset($status)){ 
                        ...
                        <tr>
                            ...
                            <td><a href="javascript:void(0)" data-id="<?= $val->id ?>" class="btn btn-primary edit_form "><i class="fa fa-edit"></i></a>
                                <button value='{{$val->id}}' data="{{$val->status}}" data-name="purchase_mode" class='btn btn-info change'>status</button></td>
...
<script>
        ...
        messages: {
            purchase_mode: 'This Purchase Mode is Required',
            ...
            $.ajax({
                ...
                success: function(data) {   
                    if(data != 1){
                        ...
                        $("#purchase_mode").val(row.purchase_mode);
                        $("#status").val(row.status).select2();     
                        $("#id").val(row.id);           
                        $(".card-title").html('Purchase Mode').css('color','blue');                  
                        $("#purchase_mode").focus();
                        scrollToTop();
                    }               
                },
