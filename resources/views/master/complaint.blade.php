                    ...
                    <div class="card border-left-primary shadow h-100 ">
                        ...
                        <div class="card-body">
                            ...
                            <form id="form" method="post" action="{{route('status.storeStatus')}}">
                                ...
                                <div class="row">
                                    <input type="hidden" name="table" value="Complaint" />
                                    <div class="col-md-4">
                                        <label for="">Complaint Status<span class="error">*</span></label>
                                        <div class="form-group">
                                            <input type="Text" placeholder="Complaint Status" id='complaint_status' class="form-control txt-num @error('complaint_status') is-invalid @enderror" name="complaint_status" />
                       ...
                       <?php if(isset($status)){ 
                        ...
                        <tr>
                            ...
                            <td><a href="javascript:void(0)" data-id="<?= $val->id ?>" class="btn btn-primary edit_form"><i class="fa fa-edit"></i></a>
                                <button value='{{$val->id}}' data="{{$val->status}}" data-name="Complaint" class='btn btn-info change'>status</button></td>
...
<script>
  $(".change").click(function(){    
        ...
        messages: {
            complaint_status: 'This Complaint Status is Required',
            ...
            $.ajax({
                ...
                success: function(data) {   
                    if(data != 1){
                        ...
                        $("#complaint_status").val(row.complaint_status);
                        $("#status").val(row.status).select2();     
                        $("#id").val(row.id);           
                        $(".card-title").html('Complaint Status').css('color','blue');                  
                        $("#complaint_status").focus();
                        scrollToTop();
                    }               
                },
                error: function(data) {
