                ...
                <div class="col-sm-12">
                    <div class="card border-left-primary shadow h-100 ">
                        <div class="card-header">
                            <h3 class="card-title">Courier Management</h3>
                        </div>
                        <div class="card-body">
                            @if (session('msg'))
                            ...
                            <form id="form" method="post" action="{{route('status.storeStatus')}}">
                                ...
                                <div class="row">
                                    <input type="hidden" name="table" value="Courier" />
                                    <div class="col-md-4">
                                        <label for="">Courier Name<span class="error">*</span></label>
                                        <div class="form-group">
                                            <input type="Text" id='courier_name' class="form-control txt-num @error('courier_name') is-invalid @enderror" name="courier_name" placeholder="Courier Name"/>
                                            @error('courier_name')
                       ...
                       <?php if(isset($status)){ 
                        ...
                        <tr>
                            ...
                            <td><a href="javascript:void(0)" data-id="<?= $val->id ?>" class="btn btn-primary edit_form"><i class="fa fa-edit"></i></a>
                                ...
                                <button value='{{$val->id}}' data="{{$val->status}}" data-name="Courier" class='btn btn-info change'>status</button>
                            </td>
                        </tr>
                        <?php $i++; }  } ?>
                    </tbody>
...
<script>
  $(".change").click(function(){    
        ...
        messages: {
            courier_name: 'This Courier Name is Required',
            ...
            $.ajax({
                ...
                success: function(data) {   
                    if(data != 1){
                        ...
                        $("#courier_name").val(row.courier_name);
                        $("#status").val(row.status).select2();     
                        $("#id").val(row.id);           
                        $(".card-title").html('Edit Courier Management').css('color','blue');                  
                        $("#courier_name").focus();
                        scrollToTop();
                    }               
                },
